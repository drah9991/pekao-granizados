...
-- Description: Evita que el `store_stock` de los productos paramétricos como
--              "granizado" o volumétricos caigan a números negativos durante
    ...
    LOOP
        ...
        BEGIN
            ...
            IF NOT COALESCE(is_tracked_mixture, false) THEN
                ...
                VALUES (item_row.product_id, v_old_store_id, 'entry', item_row.qty, 'Corrección Pedido #' || substring(v_order_id::text from 1 for 8), auth.uid());
            END IF;
        ...
        BEGIN
            ...
            IF NOT COALESCE(is_tracked_mixture, false) THEN
                ...
                VALUES (
                    ...
                    'ANULACIÓN #' || substring(p_order_id::text from 1 for 8) || ' — ' || item_row.name || ' | Motivo: ' || trim(p_reason),
                    auth.uid()
                );
            END IF;

-- ==============================================================================
-- Migration: Inventory Realtime Sync & Lock Ordering Optimization
-- Description: Sorts product loops and recipe loops to prevent deadlocks,
--              delegates tank volume sync entirely to inventory triggers,
--              fixes check_inventory_stock_trigger to reference NEW.unit,
--              and adds index optimizations.
-- ==============================================================================

-- 1. Recreate process_sale
...
BEGIN
    ...
    LOOP
        ...
        IF v_item.product_id IS NOT NULL THEN
            ...
            -- Trigger trg_sync_inventory_to_machine_tanks handles machine_tanks update.
...
-- 2. Recreate update_order_with_stock
...
-- 3. Recreate cancel_sale_with_stock_restore
...
-- 4. Recreate check_inventory_stock_trigger function
...
BEGIN
    ...
    IF (NEW.stock <= NEW.min_stock) AND (OLD.stock > NEW.min_stock OR OLD.stock IS NULL) THEN
        ...
        VALUES (
            ...
            'El artículo "' || NEW.name || '" tiene un stock de ' || NEW.stock || ' ' || NEW.unit || '. El mínimo es ' || NEW.min_stock || '.',
            'inventory_low',
            'high',
...
CREATE INDEX IF NOT EXISTS idx_machine_tanks_item ON public.machine_tanks(inventory_item_id, store_id);
DROP INDEX IF EXISTS public.idx_store_stock_store_product;

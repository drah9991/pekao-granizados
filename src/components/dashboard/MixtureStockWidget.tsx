...
export const MixtureStockWidget = React.memo(function MixtureStockWidget({ data }: MixtureStockWidgetProps) {
  if (!data || !data.lowStock) return null;

  // Filter to show only real mixtures in this specific widget
    ...
    return (
      <Card className="col-span-1 md:col-span-2 lg:col-span-2 glass-pro border-border dark:border-white/5 rounded-[2.5rem] shadow-sm dark:shadow-pro overflow-hidden relative group animate-pro-in">
        ...
        <CardContent className="p-6">
          <div className="py-20 text-center space-y-4 opacity-30 border-2 border-dashed border-border rounded-3xl">
    ...
    <Card className="col-span-1 md:col-span-2 lg:col-span-2 glass-pro border-border dark:border-white/5 rounded-[2.5rem] shadow-sm dark:shadow-pro overflow-hidden relative group animate-pro-in">
      ...
      <CardContent className="p-6 space-y-6">
        <div className="space-y-4">
          {mixtures.map((item: any) => {
            const liters = item.stock / 1000;
            const servings = Math.floor(Math.max(0, item.stock) / STANDARD_SERVING_ML);
            const isLow = item.stock <= (item.min_stock || 0);
            const rawPercentage = (item.stock / (item.min_stock * 3 || 5000)) * 100;
                ...
                <div className="grid grid-cols-1 gap-4 relative z-10">
                  <div className="flex flex-wrap gap-2">
                    ...
                    {availableSizes.slice(0, 2).map((size: any, sIdx: number) => {
                      ...
                      const sizeServings = Math.floor(Math.max(0, item.stock) / (STANDARD_SERVING_ML * size.multiplier));
                      return (
                        <div key={sIdx} className="px-3 py-1.5 bg-white/5 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 flex items-center gap-2">
